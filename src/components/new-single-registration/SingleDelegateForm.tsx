...
const SingleDelegateForm: React.FC<SingleDelegateFormProps> = ({
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/coupons/validate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code: couponCode.trim().toUpperCase(), registrationType: "single" })
      });
      const data = await res.json();
      if (data.success) {
    ...
    if (isSuccess) {
      setTimeout(() => {
        ...
        if (el) {
          ...
          window.scrollTo({ top: 500, behavior: "smooth" }); // Fallback offset
        }
      }, 100);
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            if (verifyData.success) {
              // 4. If file is uploaded, upload it to Cloudinary first
              let docUrl = "";
              if (uploadedFile) {
                try {
              ...
              const regRes = await fetch(`${API_URL}/delegates-registration/verify`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            setSubmitError("Failed to submit registration. Please check your connection.");
          } finally {
            setIsSubmitting(false);
          }
        },
  ...
  return (
    ...
    >
      ...
      <motion.form variants={formContainerVariants} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-50px" }} onSubmit={handleSubmit} className="px-6 pb-6 pt-3 md:px-8 md:pb-8 md:pt-4 flex flex-col gap-8 relative overflow-hidden">
        
        {/* Personal Information */}
        <div className="flex flex-col gap-5">
          <motion.div variants={formRowVariants} className="flex justify-between items-center border-b border-gray-100 pb-2 -ml-1 relative after:absolute after:bottom-0 after:left-1.5 after:w-12 after:h-[2.5px] after:bg-[#d18e26]">
          ...
          <motion.div variants={formRowVariants} style={{ transformOrigin: "top center", transformStyle: "preserve-3d" }} className="grid grid-cols-1 md:grid-cols-12 gap-5">
            <motion.div variants={formFieldVariants} className="md:col-span-3 flex flex-col gap-1.5">
            ...
            <motion.div variants={formFieldVariants} className="md:col-span-9 flex flex-col gap-1.5">
          ...
          <motion.div variants={formRowVariants} style={{ transformOrigin: "top center", transformStyle: "preserve-3d" }} className="grid grid-cols-1 md:grid-cols-2 gap-5 relative">
            ...
            <motion.div variants={formFieldVariants} className="flex flex-col gap-1.5 relative">
              ...
              <AnimatePresence>
                {mobileOtpSent && !mobileOtpVerified && (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs text-amber-900 font-medium">
                      <span>Enter 6-digit OTP sent to your Mobile</span>
                    </div>
                    <div className="flex gap-2">
                      <input
                        type="text"
        ...
        <div className="flex flex-col gap-5 mt-0">
          <motion.div variants={formRowVariants} className="flex items-center gap-2 border-b border-gray-100 pb-2 -ml-1 relative after:absolute after:bottom-0 after:left-1.5 after:w-12 after:h-[2.5px] after:bg-[#d18e26]">
            <div className="bg-[#f0f7f0] p-1.5 rounded-md text-[#2b5922]">
              <ShieldCheck size={18} />
            </div>
          ...
          <motion.div variants={formRowVariants} style={{ transformOrigin: "top center", transformStyle: "preserve-3d" }} className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <motion.div variants={formFieldVariants} className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-black flex items-center justify-between">
                ...
                {loadingCategories && <Loader2 size={12} className="animate-spin text-gray-400" />}
              </label>
          ...
          <motion.div variants={formRowVariants} style={{ transformOrigin: "top center", transformStyle: "preserve-3d" }}>
            <motion.div variants={formFieldVariants} className="flex flex-col gap-1.5 relative">
          ...
          <motion.div variants={formRowVariants} style={{ transformOrigin: "top center", transformStyle: "preserve-3d" }}>
            <motion.div variants={formFieldVariants} className="flex flex-col gap-1.5 mt-2">
        ...
        <div className="flex flex-col gap-3 -mt-2">
          ...
          <div className="flex items-center justify-between pt-1 relative z-10">
            ...
            <button type="submit" disabled={isSubmitting} className="bg-[#2b5922] hover:bg-[#1a3d14] text-white px-6 py-3 rounded-lg font-bold text-xs uppercase tracking-wider transition-all shadow-md hover:shadow-lg flex items-center gap-2 group disabled:opacity-50 cursor-pointer">
              {isSubmitting ? (
                <>
                ...
                <>
                  Submit Registration <ArrowRight size={15} className="group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>
          </div>
