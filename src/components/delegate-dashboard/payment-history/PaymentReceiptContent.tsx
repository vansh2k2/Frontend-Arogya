...
export const PaymentReceiptContent: React.FC<PaymentReceiptContentProps> = ({
  delegate,
  onBack,
}) => {
  ...
  return (
    <div className="space-y-4 font-inter w-full antialiased pb-10">
      <style>{`
        @media print {
          @page {
            size: A4 portrait;
      ...
      {/* ── Top Bar Controls: Back & Quick Actions ── */}
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-xs print:hidden">
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold transition-all cursor-pointer"
        >
          <ArrowLeft size={16} />
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={handlePrint}
            className="flex items-center gap-1.5 px-3.5 py-1.5 bg-[#0f4d1e] hover:bg-[#093514] text-white rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer"
          >
            <Printer size={14} />
      ...
      {/* ── Full Page Printable Receipt Paper Frame (Thin Light Green Border) ── */}
      <div
        ref={receiptRef}
        className="printable-receipt-area bg-white text-slate-900 pt-2 sm:pt-3 px-4 sm:px-6 pb-4 sm:pb-6 space-y-3 print:p-0 print:shadow-none relative border border-[#15803d]/30 rounded-xl max-w-4xl mx-auto shadow-sm"
      >
